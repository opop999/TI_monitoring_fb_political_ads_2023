...
output: 
  flexdashboard::flex_dashboard:
    ...
    source_code: https://github.com/opop999/TI_monitoring_fb_political_ads_2023
    navbar:
      - {title: "Show all", icon: "ion-ios-people", href: "./index.html"}
...
params:
  ...
  data_dir: "output"
  plots_dir: "plots"
  candidates_dir: "input"
  plots_static_dir: "static_candidates"
  plots_interactive_dir: "interactive_candidates"
  spenders_limit_high: 100
...
# Get election candidates ids for filtering
candidates_id <- readRDS(file.path(params$candidates_dir, "numeric_ids_candidates.rds"))

# Import datasets created in the previous script
summary_dataset <- readRDS(file.path(params$data_dir, "summary_tables", "merged_summary.rds")) %>% filter(page_id %in% candidates_id)
time_dataset <- readRDS(file.path(params$data_dir, "summary_tables", "time_summary.rds")) %>% filter(page_id %in% candidates_id)
regional_spenders <- readRDS(file.path(params$data_dir, "summary_tables", "regional_spenders.rds")) %>% filter(page_id %in% candidates_id)
...
y_lab <- element_blank()
title_lab <- paste(
  "Total ads of presidential candidates since",
...
```{r include=FALSE}
# Save plots
ggsave(paste0(file.path(static_dir, plot_name), ".png"), plot, "png", bg = "white")
...
y_lab <- element_blank()
title_lab <- paste(
  "Spending on ads of presidential candidates since",
  format(start_date, "%d.%m.%Y")
)
```
...
y_lab <- element_blank()
title_lab <- paste(
  "Average ad length in words of presidential candidates since",
  format(start_date, "%d.%m.%Y")
)
```
...
y_lab <- element_blank()
title_lab <- paste(
  "Average ad runtime of presidential candidates since",
  format(start_date, "%d.%m.%Y")
)
```
...
y_lab <- element_blank()
title_lab <- paste(
  "Total ads impressions of presidential candidates since",
  format(start_date, "%d.%m.%Y")
)
```
...
y_lab <- element_blank()
title_lab <- paste(
  "Average minimal estimated audience of presidential candidates since",
  format(start_date, "%d.%m.%Y")
)
```
...
y_lab <- "CZK thousand"
title_lab <- paste(
  "Weekly spending on ads of presidential candidates since",
  format(start_date, "%d.%m.%Y")
)
```
...
y_lab <- element_blank()
title_lab <- paste(
  "Gender targeting of presidential candidates since",
  format(start_date, "%d.%m.%Y")
)
```
...
y_lab <- element_blank()
title_lab <- paste(
  "Age of audience of presidential candidates since",
  format(start_date, "%d.%m.%Y")
)
```
...
y_lab <- element_blank()
title_lab <- paste(
  "Regions of audience of ads of presidential candidates since",
  format(start_date, "%d.%m.%Y")
)
```
...
y_lab <- "CZK thousand"
title_lab <- paste(
  "Spending of presidential candidates by region since",
  format(start_date, "%d.%m.%Y")
)
```
