...
get_all_tables_merge <- function(token, account_ids, min_date, max_date, regions, dir_name = "output", old_df = NA, new_df = "merged_dataset") {
  ...
  if (!is.na(old_df)) {
    existing_fb_dataset <- readRDS(paste0(file.path(dir_name, old_df), ".rds"))
  ...
  saveRDS(merged_dataset, paste0(file.path(dir_name, new_df), ".rds"))
  fwrite(merged_dataset, paste0(file.path(dir_name, new_df), ".csv"))
...
get_all_tables_merge(
  ...
  account_ids = readRDS("input/numeric_ids_all_saved_pages.rds"),
  min_date = "2022-07-01",
